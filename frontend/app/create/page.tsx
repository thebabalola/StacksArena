...
export default function CreateVaultPage() {
  ...
  const amountInSTX = useMemo(() => {
    ...
    return (Number(amountMicroStx) / 1_000_000).toLocaleString(undefined, {
      maximumFractionDigits: 6,
    });
  }, [amountMicroStx]);

  const exceedsBalance =
    amountMicroStx > rawMicroStx && rawMicroStx > BigInt(0);
  ...
  return (
    <div className="min-h-screen px-6 py-12 bg-background transition-colors duration-300">
      <div className="mx-auto max-w-3xl">
        <Link
          href="/"
          className="flex items-center gap-2 text-xs text-slate-500 hover:text-primary transition-colors mb-8 uppercase font-black tracking-widest"
        >
        ...
        >
          ...
          <p className="text-slate-400 font-bold uppercase text-sm tracking-wide">
            Define your lock conditions and secure your assets under
            Bitcoin-anchored rules.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-2">
            <form
              ...
              className="space-y-8 glass-panel p-8 rounded-3xl border border-white/5 shadow-2xl relative overflow-hidden"
            >
              ...
              <div className="space-y-3 relative z-10">
                <div className="flex items-center justify-between">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 flex items-center gap-2">
                    <Zap className="w-3 h-3 text-primary" /> Commitment Amount
                    (STX)
                  </label>
                  {isConnected && (
                    <span
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 flex items-center gap-2">
                    <Clock className="w-3 h-3 text-primary" /> Lock Duration
                    (Blocks)
                  </label>
                  <input
                    type="number"
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 flex items-center gap-2">
                    <Scale className="w-3 h-3 text-primary" /> Early Access
                    Penalty (%)
                  </label>
                  <input
                    type="number"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 flex items-center gap-2">
                    <Users className="w-3 h-3 text-primary" /> Multi-Sig
                    Threshold
                  </label>
                  <input
                    type="number"
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 flex items-center gap-2">
                    <LayoutDashboard className="w-3 h-3 text-primary" /> Phased
                    Milestones
                  </label>
                  <input
                    type="number"
              ...
              <div className="space-y-3 relative z-10">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 flex items-center gap-2">
                  <Box className="w-3 h-3 text-primary" /> SIP-010 Token Address
                  (Optional)
                </label>
                <input
                  type="text"
          ...
          <div className="space-y-6">
            <div className="glass-panel p-6 rounded-3xl border border-white/5 space-y-4">
              ...
              <ul className="space-y-3">
                {[
                  ...
                  <li
                    key={rule}
                    className="flex items-center gap-2 text-[10px] font-bold text-slate-500 uppercase tracking-tight"
                  >
            ...
            <div className="glass-panel p-6 rounded-3xl border border-white/5 bg-gradient-to-br from-companion/5 to-transparent">
              ...
              <p className="text-[10px] font-bold text-slate-500 leading-relaxed uppercase">
                Assets are held in a non-custodial smart contract. Only the
                defined conditions can trigger a release.
              </p>
            </div>
          </div>
        </div>
